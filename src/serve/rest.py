...
def get_production_model(model_name):
    ...
    for version in results:
        if version.current_stage == 'Production':
            local_dir = "C:/Users/ID/Desktop/FAKULTET/1 LETNIK MAG/2semestar/IIR/gold-price-prediction/models/"
...
def predict():
    ...
    input_data = {col: predict_request[col].values.reshape(-1, 1).astype(
        np.float32) for col in predict_request.columns}
